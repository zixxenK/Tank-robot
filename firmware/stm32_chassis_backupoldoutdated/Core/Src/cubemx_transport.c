...
static volatile uint8_t s_last_error = CUBEMX_TRANSPORT_ERR_NOT_INITIALIZED;
...
void cubemx_transport_init(UART_HandleTypeDef *huart)
{
  ...
  s_rx_tail = 0U;
  s_rx_active = 0U;
  s_last_error = CUBEMX_TRANSPORT_OK;
...
bool cubemx_transport_open(struct uxrCustomTransport *transport)
{
  ...
  {
    s_last_error = CUBEMX_TRANSPORT_ERR_NOT_INITIALIZED;
    return false;
  }
  ...
  {
    s_rx_active = 0U;
    s_last_error = CUBEMX_TRANSPORT_ERR_RX_START_FAILED;
    return false;
  }
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
  if (huart == s_transport_huart && s_rx_active != 0U)
  {
    ...
    {
      s_rx_active = 0U;
      s_last_error = CUBEMX_TRANSPORT_ERR_UART_ERROR_REARM;
      return;
    }
