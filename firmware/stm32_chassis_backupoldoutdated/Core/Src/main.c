...
IWDG_HandleTypeDef hiwdg;   // Independent watchdog for fail-safe reset
...
#if BRINGUP_MODE_BLINK_ONLY
static void run_blink_only_loop(void);
#endif
...
static void send_emergency_stop_notification(void);
...
void register_external_motor_command(int16_t left_speed, int16_t right_speed);
...
int main(void)
{
  ...
  s_last_imu_retry_ms = HAL_GetTick();
  s_imu_ready = (MPU6050_Init(&hi2c1) == HAL_OK) ? 1u : 0u;
  update_motor_pwm();
...
static void run_blink_only_loop(void)
{
  while (1)
  {
    ...
    HAL_Delay(BRINGUP_BLINK_PERIOD_MS);
  }
}
#endif
...
{
  uint8_t frame[8u];
  size_t frame_len = protocol_build_frame(PROTO_FUNC_EMERGENCY_STOP,
                                          NULL,
                                          0u,
                                          frame,
                                          sizeof(frame));
...
void imu_service(void)
{
  ...
  if (s_imu_ready == 0u)
  {
    if ((now - s_last_imu_retry_ms) >= IMU_RETRY_INTERVAL_MS)
    {
      s_last_imu_retry_ms = now;
  ...
  if ((now - s_last_imu_poll_ms) < IMU_POLL_INTERVAL_MS)
  {
    return;
  }

  s_last_imu_poll_ms = now;
  if (MPU6050_ReadData(&hi2c1, &s_imu_data) != HAL_OK)
  {
    s_imu_ready = 0u;
    s_last_imu_retry_ms = now;
  }
...
/**
  * @brief IWDG Initialization for fail-safe firmware reset.
  * @retval None
  */
static void MX_IWDG_Init(void)
{
...
static void MX_GPIO_Init(void)
{
...
#if BRINGUP_BLINK_GPIO_ENABLE
  GPIO_InitTypeDef blinkGpio = {0};

  __HAL_RCC_GPIOC_CLK_ENABLE();

  blinkGpio.Pin = BRINGUP_LED_PIN;
  blinkGpio.Mode = GPIO_MODE_OUTPUT_PP;
  blinkGpio.Pull = GPIO_NOPULL;
  blinkGpio.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(BRINGUP_LED_PORT, &blinkGpio);

  HAL_GPIO_WritePin(BRINGUP_LED_PORT, BRINGUP_LED_PIN, GPIO_PIN_RESET);
#endif

#if MOTOR_DIRECTION_GPIO_ENABLE
  GPIO_InitTypeDef motorGpio = {0};

  __HAL_RCC_GPIOA_CLK_ENABLE();
  __HAL_RCC_GPIOB_CLK_ENABLE();

  motorGpio.Pin = MOTOR_LEFT_FWD_PIN | MOTOR_LEFT_REV_PIN |
                        MOTOR_RIGHT_FWD_PIN;
  ...
  motorGpio.Pin = MOTOR_RIGHT_REV_PIN;
  motorGpio.Mode = GPIO_MODE_OUTPUT_PP;
  motorGpio.Pull = GPIO_NOPULL;
  motorGpio.Speed = GPIO_SPEED_FREQ_LOW;
