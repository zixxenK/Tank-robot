...
#if __has_include("geometry_msgs/msg/twist.h") && \
  __has_include("rcl/rcl.h") && \
  __has_include("rclc/executor.h") && \
  __has_include("rclc/rclc.h") && \
  __has_include("rmw_microros/rmw_microros.h")
#define MICRO_ROS_STACK_AVAILABLE 1
#else
#define MICRO_ROS_STACK_AVAILABLE 0
#endif
#else
#define MICRO_ROS_STACK_AVAILABLE 0
#endif

#if MICRO_ROS_STACK_AVAILABLE
...
extern void motor_safety_check(void);
extern void send_heartbeat(void);
extern void register_external_motor_command(int16_t left_speed, int16_t right_speed);
extern void imu_service(void);
...
static void stop_motors(void)
{
  ...
  register_external_motor_command(0, 0);
}
...
static void apply_cmd_vel(const geometry_msgs__msg__Twist *msg)
{
  ...
  register_external_motor_command((int16_t)(left_norm * (float)MOTOR_PWM_MAX_DUTY),
                                  (int16_t)(right_norm * (float)MOTOR_PWM_MAX_DUTY));
}
...
static void send_transport_status(uint8_t status_code, uint8_t detail)
{
  uint8_t payload[2u] = {status_code, detail};
  uint8_t frame[10u];
  size_t frame_len = protocol_build_frame(PROTO_FUNC_SYS,
                                          payload,
                                          2u,
                                          frame,
                                          sizeof(frame));
  if (frame_len > 0u)
  {
    (void)HAL_UART_Transmit(&huart2, frame, (uint16_t)frame_len, 50u);
  }
}
...
void StartDefaultTask(void *argument)
{
  ...
  uint32_t last_transport_retry_ms = 0u;
  uint32_t last_status_report_ms = 0u;
  uint8_t transport_started = 0u;
  ...
  for (;;)
  {
    ...
    if (transport_started == 0u &&
        (now_ms - last_transport_retry_ms) >= 250u)
    {
      last_transport_retry_ms = now_ms;
      if (cubemx_transport_open(NULL))
      {
        transport_started = 1u;
        send_transport_status(0x01u, CUBEMX_TRANSPORT_OK);
      }
    }

    if (transport_started != 0u && !cubemx_transport_is_active())
    {
    ...
    if ((now_ms - last_status_report_ms) >= 1000u)
    {
      ...
      send_transport_status(transport_started ? 0x01u : 0xE1u,
                            cubemx_transport_last_error());
    }
