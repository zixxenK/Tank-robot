...
"@
  ...
  # Keep the ROS workspace owned by rock64 and let the workflow's individual
  # privileged operations use passwordless sudo.  Running the whole workflow
  # as root would redirect rosdep to /root and create root-owned build files.
  $remoteCommand = "STM32_BUILD_JOBS=4 FLASH_ESP32=false /bin/bash '$RemoteRoot/deployment/scripts/rock64_update_and_flash.sh'"
