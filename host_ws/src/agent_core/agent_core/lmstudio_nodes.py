...
class _LMStudioNode(Node):
    ...
    def __init__(self, name: str) -> None:
        ...
        self._client = LMStudioClient(
            str(self.get_parameter("base_url").value),
            str(self.get_parameter("model").value),
            float(self.get_parameter("request_timeout").value),
        )
...
class TeleopChatNode(_LMStudioNode):
    ...
    def _request_callback(self, message: String) -> None:
        if message.data.strip().lower() in {"stop", "halt", "cancel"}:
            self._stop("operator stop")
            return
        try:
            arguments = self._client.call_tool(message.data, _MOVE_TOOL)
